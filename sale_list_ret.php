...
$oid=rawurldecode($_REQUEST['oid']);
if($brncd==""){$brncd1="";}else{$brncd1=" and bcd='$brncd'";}
if($invtyp==""){$invtyp1="";}else{$invtyp1=" and cstat='$invtyp'";}
if($grp==""){$grp1="";}else{$grp1=" and grp='$grp'";}
if($oid==""){$oid1="";}else{$oid1=" and oid like '%$oid%'";}

if($catsl==""){$catsl1="";}else{$catsl1=" and cat='$catsl'";}
if($prnm==""){$prnm11="";}else{$prnm11=" and prsl='$prnm'";}
...
<div class="row">
<div class="col-md-12">
<div class="col-md-12" style="background-color:#000"><font size="5" color="#fff">Sale Details</font></div>
 <table  width="100%" class="advancedtable"  >
			...
			<td  align="center" ><b>Original Inv No.</b></td>
			<td  align="center" ><b>Original Inv Date</b></td>
...
$cgst1=0;
$sgst1=0;
$data1= mysqli_query($conn,"select * from  main_billing_ret where sl>0".$todts.$snm1.$brncd1.$invtyp1.$grp1.$oid1." order by dt,sl")or die(mysqli_error($conn));
...
$datad= mysqli_query($conn,"select * from main_ledg where sl='$grp2'")or die(mysqli_error($conn));
while ($rowd = mysqli_fetch_array($datad))
{
$grp22=$rowd['nm'];
}
...
$total_am=0;
$disa_am=0;
$data= mysqli_query($conn,"select * from  main_billdtls_ret where sl>0 and blno='$blno'".$pqq.$catsl1.$prnm11)or die(mysqli_error($conn));
	...
	if($blno==$blno1)
	{
		$asd++;
		 ...
		 ?>
			...
			<?if($asd==1){?>
			<td  align="center"  ><?=$sln;?><br/>
			</td>
			<td  align="center" ><?=$dt;?></td>
			<td  align="center" ><?=$inv;?></td>
			<td  align="center" ><?=$invdt;?></td>
