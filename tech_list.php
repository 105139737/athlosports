...
$datar= mysqli_query($conn,"select * from ".$DBprefix."catg where sl>0 $all1 $sec1")or die(mysqli_error($conn));
$rcnt=mysqli_num_rows($datar);
$data= mysqli_query($conn,"select * from  ".$DBprefix."catg where sl>0 $all1 $sec1 order by cnm limit $start,$ps ")or die(mysqli_error($conn));
...
<input type="text" name="ps" id="ps" value="<?=$ps;?>" size="7" onblur="pagnt1(this.value)">
  ...
  <table  class="table table-hover table-striped table-bordered"  >
	    ...
	    <th width="20%" style="text-align:left" hidden>Section </font></th>
...
$sec_nm="";
$get=mysqli_query($conn,"select * from ".$DBprefix."sec where sl='$sec'") or die(mysqli_error($conn));
...
?>
		...
		<td align="left" hidden><? echo $sec_nm;?></td>
