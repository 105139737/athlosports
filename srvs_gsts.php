...
$query1 = "SELECT sum(ttl) as gttl,sum(tamm) as gtamm,sum(cgst_am) as cgst,sum(sgst_am) as sgst,sum(igst_am) as igst FROM main_ser_slt where eby='$user_currently_loged'";
...
{
$yyy=$yy."%";
$query51="select * from main_ser_billing where blno like '$yyy' order by sl";
...
$billno=$vid1;
$blno=$yy.$vnoc;
$query5="select * from main_ser_billing where blno='$blno'";
...
VALUES ('$vcno','$blno','$custnm','$dt','Service','$grp','-2','$gtamm','$brncd','$user_currently_loged','$dttm','1')";
...
$query211 = "INSERT INTO main_ser_billing (blno,cid,grp,amm,tamm,gstam,roff,paid,crdtp,cbnm,dt,edt,pdts,vat,vatamm,car,dis,bcd,eby,tpoint,fst,tst,gst,tmod,psup,vno,lpd,gstin,billno,sec,als) 
...
$ppdis=0;
$query100 = "SELECT * FROM main_ser_slt where eby='$user_currently_loged' order by sl";
...
$query21 = "INSERT INTO main_ser_billdtls (srtnm,betno,cat,scat,prsl,imei,unit,kg,grm,pcs,srt,adp,prc,ttl,tamm,gtamm,blno,fst,tst,cgst_rt,sgst_rt,igst_rt,cgst_am,sgst_am,igst_am,net_am,refno,usl,uval,total,disp,disa,dt,mrp,rate,stk_rate,sec) 
...
$query2 = "DELETE FROM main_ser_slt WHERE eby='$user_currently_loged'";
$result2 = mysqli_query($conn,$query2)or die (mysqli_error($conn));



 $query1 = "SELECT sum(net_am) as net_am FROM main_ser_billdtls where blno='$blno'";
   $result1 = mysqli_query($conn,$query1);
while ($R1 = mysqli_fetch_array ($result1))
{
$gttl=$R1['net_am'];
