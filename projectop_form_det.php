...
<label>
	...
	<?
		if($ldgr==4)
		{
			...
			<option value="">---Select---</option>
			<?
				$query6="select * from  main_ship ";
				$result5 = mysqli_query($conn,$query6);
				while($row=mysqli_fetch_array($result5))
				{
					?>
					<option value="<?=$row['sl'];?>"<?=$row['sl'] == $cid ? 'selected' : '' ?>><?=$row['spn'];?></option>
					<?
				}
			?>
			</select>
		...
		elseif($ldgr==12)
		{
			...
			<option value="">---Select---</option>
			<?
				$query6="select * from  main_comp ";
				$result5 = mysqli_query($conn,$query6);
				while($row=mysqli_fetch_array($result5))
				{
					?>
					<option value="<?=$row['sl'];?>"<?=$row['sl'] == $sid ? 'selected' : '' ?>><?=$row['cnm'];?></option>
					<?
				}
			?>
			</select>
