...
if($sec==""){$sec1="";}else{$sec1=" and sec='$sec'";}
...
$cgst1=0;
$sgst1=0;
$data1= mysqli_query($conn,"select * from  main_billing where sl>0".$todts.$snm1.$brncd1.$invtyp1.$sec1." order by dt,sl")or die(mysqli_error($conn));
...
$total_am=0;
$disa_am=0;
$data= mysqli_query($conn,"select * from  main_billdtls where sl>0 and blno='$blno' $catsl1 $prnm1 order by sl")or die(mysqli_error($conn));
...
?>
			...
			<td  align="center" hidden><?=$betno;?></td>
...
if($sto==0){
?>
<a class="btn btn-danger btn-xs" onclick="if(confirm('Are You Sure...')){dlt('<?=$blno;?>')}">Cancel</a>
<?}?>
	</b>
