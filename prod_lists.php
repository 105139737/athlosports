...
$cnt=0;
$sl=$start;
$datatt= mysqli_query($conn,"SELECT * FROM main_product where sl>0 $cat1 $scat1 $a2 order by sl")or die(mysqli_error($conn));
$rcntttl=mysqli_num_rows($datatt);
$datar= mysqli_query($conn,"SELECT * FROM main_product where sl>0 $cat1 $scat1 $a2 order by sl")or die(mysqli_error($conn));
$rcnt=mysqli_num_rows($datar);
$get=mysqli_query($conn,"select * from main_product where sl>0 $cat1 $scat1 $a2 order by sl limit $start,$ps") or die(mysqli_error($conn));
...
<td style="text-align:left;"><?=reformat($pnm);?></td>
...
<a href="prod_list_edit.php?sl=<?=$psl;?>&gsl=<?=$gsl;?>"  title="Click to Update"><i class="fa fa-pencil-square-o"></i></a> 
<?/*
&nbsp;&nbsp;&nbsp;
