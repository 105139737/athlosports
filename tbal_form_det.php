...
if($cust!=""){$cqw=" and (cid='$cust' or sid='$cust')";}else{$cqw="";}
if($sup!=""){$sqw=" and (sid='$sup' or cid='$sup')";}else{$sqw="";}
if($pno!="0"){$pno1=" and pno='$pno'";}else{$pno1="";}

if($ledg==12 or $ledg==4)
{
$dld=" and (dldgr='$ledg' or dldgr='$ledg')";
$cld=" and (cldgr='$ledg' or cldgr='$ledg')";
}
else
{
$dld=" and dldgr='$ledg'";
...
$query = "SELECT sum(amm) as amm1 FROM main_drcr where dt between '1970-01-01' and '$prevdt' $dld".$sqw.$cqw.$pno1.$brncd1;
...
$a='';
$p='0';
$query = "SELECT * FROM main_drcr where  dt between '$fdt1' and '$tdt1' $sqw $cqw $pno1 $brncd1 $dld order by dt";
$result = mysqli_query($conn,$query);
...
$CR=0;
$f=0;
$query = "SELECT sum(amm) as amm1 FROM main_drcr where dt between '1970-01-01' and '$prevdt' $cld".$sqw.$cqw.$pno1.$brncd1;
$result = mysqli_query($conn,$query);
...
$a='';
$p='0';
$query = "SELECT * FROM main_drcr where  dt between '$fdt' and '$tdt' $sqw $cqw $pno1 $brncd1 $cld order by dt";
$result = mysqli_query($conn,$query);
while ($R = mysqli_fetch_array ($result))
{
...
<tr class="even">
       <td width="100%"  colspan="8" align="right" ><input type="submit" class="btn btn-primary" value=" Export to Excel "></td>
 </tr>
</table>
