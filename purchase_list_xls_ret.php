...
$file="Purchase Return Report $fdt to $tdt.xls";
...
{
$data1= mysqli_query($conn,"select * from main_purchase_ret where sl>0".$todt.$snm1.$brncd1.$ptyp1.$vstat1." order by dt,sl")or die(mysqli_error($conn));
}
else
{
$data1= mysqli_query($conn,"select * from main_purchase_ret where sl>0 ".$todt.$snm1.$brncd1.$ptyp1.$vstat1." order by dt,sl")or die(mysqli_error($conn));
...
$tigst_am=0;
$tgst=0;
$data= mysqli_query($conn,"select * from  main_purchasedet_ret where sl>0 and blno='$blno' $prnm1 $catsl1")or die(mysqli_error($conn));
