...
$stk=0;
$rest5 = mysqli_query($conn,"select * from ".$DBprefix."stock where refno='$blno' and pcd='$prsl' and bcd='$bcd'");
$stk=mysqli_num_rows($rest5);
?>
<tr class="even">
<td align="left" width="10%" ><b><?=$cnm;?></b></td>

<td align="left" width="10%" <?if($stk==1){?>onclick="get_data('<?=$tsl;?>','<?=$prsl;?>','<?=$unit;?>','<?=$qty;?>','<?=$mrp;?>','<?=$total;?>','<?=$disp;?>','<?=$disa;?>','<?=$ttl;?>','<?=$cgst_rt;?>','<?=$cgst_am;?>','<?=$sgst_rt;?>','<?=$sgst_am;?>','<?=$igst_rt;?>','<?=$igst_am;?>','<?=$rate;?>','<?=$net_am;?>','<?=$unit;?>','<?=$betno;?>','<?=$bcd;?>','<?=$cat;?>','<?=$scat;?>','<?=$pnm;?>')" style="cursor:pointer;"<?}?>>
<b><?=$pnm;?></b>
</td>
<td align="center" width="5%" ><b><?=$unit_nm;?></b></td>
<td align="center" width="7%" style="display:none;"><b><?=$betno;?></b></td>
...
<td align="center" width="3%"><b><?if($stk==1){?><a onclick="if(confirm('Are you Sure?')){deltpr1('<?=$tsl;?>')}"><font color="red">Delete</font></a><?}?></b></td>
</tr>
<?
$total_am+=$total;
$total_cgst+=$cgst_am;
