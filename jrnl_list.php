...
$sl=$start;
$sln=0;
$datatt= mysqli_query($conn,"SELECT * FROM main_drcr where typ='2' and stat='1'".$brncd1.$pnoo)or die(mysqli_error($conn));
$rcntttl=mysqli_num_rows($datatt);
$datar= mysqli_query($conn,"SELECT * FROM main_drcr where typ='2' and stat='1'".$pnoo.$brncd1)or die(mysqli_error($conn));
$rcnt=mysqli_num_rows($datar);
 $data= mysqli_query($conn,"select * from main_drcr where typ='2' and stat='1' $pnoo $brncd1 order by dt Desc limit $start,$ps")or die(mysqli_error($conn));
