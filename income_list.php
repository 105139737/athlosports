...
$sl=$start;
$sln=0;
$datatt= mysqli_query($conn,"SELECT * FROM main_drcr where typ='33' and stat='1'".$brncd1.$pnoo)or die(mysqli_error($conn));
$rcntttl=mysqli_num_rows($datatt);
$datar= mysqli_query($conn,"SELECT * FROM main_drcr where typ='33' and stat='1'".$pnoo.$brncd1)or die(mysqli_error($conn));
$rcnt=mysqli_num_rows($datar);
 $data= mysqli_query($conn,"SELECT * FROM main_drcr where typ='33' and stat='1' $pnoo $brncd1 order by dt Desc limit $start,$ps")or die(mysqli_error($conn));
...
<td align="center" valign="top"><?php if($path!=""){echo "<a href='$path' target='_blank'>Click to Download</a>";};?></td>
	...
	<a href="#" onclick="sfdtl3income('<? echo $sl1; ?>')" title="Edit"><img src="images/edit.png" width="30"/></a>
...
?>
<div align="center"><input type="text" size="10" id="pgn" name="pgn" value="<? echo $pnog;?>"><input Type="button" value="Go" onclick="pagnt1('')"></div>
